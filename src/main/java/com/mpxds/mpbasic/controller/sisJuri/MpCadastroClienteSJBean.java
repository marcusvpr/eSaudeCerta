...
public class MpCadastroClienteSJBean implements Serializable {
	...
	public void onCepWebService() {
    	//
		MpCepXML mpCepXML = MpClienteWebService.executaCep(this.mpClienteSJ.getMpEnderecoLocalX().getCep());
		if (null == mpCepXML)
...
//				this.mpClienteSJ.getMpEnderecoEntrega().getCep() + " / " + mpCepXML.getLogradouro());
			//
			this.mpClienteSJ.getMpEnderecoLocalX().setLogradouro(mpCepXML.getLogradouro());
			this.mpClienteSJ.getMpEnderecoLocalX().setComplemento(mpCepXML.getComplemento());
			this.mpClienteSJ.getMpEnderecoLocalX().setBairro(mpCepXML.getBairro());
			this.mpClienteSJ.getMpEnderecoLocalX().setCidade(mpCepXML.getCidade());
			// Trata UF! ...
			...
			this.mpClienteSJ.getMpEnderecoLocalX().setMpEstadoUF(mpEstadoUF);
			//
		}
    }
...
//    	MpAppUtil.PrintarLn("MpCadastroCliente.findByNome() = TRUE");
	...
	private void limpar() {
		...
		this.mpClienteSJ.setMpEnderecoLocalX(mpEnderecoLocalX);
		//
		this.limparArquivo();
	}
