...
public class MpRelatorioProcessosBean implements Serializable {
    ...
    public void geraDTO() { 
		//
		Calendar datX = Calendar.getInstance();

		datX.setTime(this.dataInicio);
		datX.set(Calendar.HOUR, 0);
		datX.set(Calendar.MINUTE, 0);
		datX.set(Calendar.SECOND, 0);
		this.dataInicio = datX.getTime();

		datX.setTime(this.dataFim);
		datX.set(Calendar.HOUR, 0);
		datX.set(Calendar.MINUTE, 0);
		datX.set(Calendar.SECOND, 0);
		this.dataFim = datX.getTime();
		//
		List<MpProcessoAndamento> mpProcessoAndamentoList = this.mpProcessoAndamentos.findAllByDtHr(this.dataInicio,
																									this.dataFim);
		//
		for (MpProcessoAndamento mpProcessoAndamento : mpProcessoAndamentoList) {
    		//
//    		System.out.println("ReportAgendaVM2.showReport() 000 ( " + 
//    				objetoProcesso.getDataCadastro() + " / " + this.datIni + " / " + this.datFim);
    		//
	    		//
//	    		System.out.println("ReportAgendaVM2.showReport() 001 ( " + 
//	    															objetoProcesso.getDataCadastro());
				//
				String responsavelX = "";
